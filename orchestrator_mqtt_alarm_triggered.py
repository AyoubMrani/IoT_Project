...
VERSION: ALARM TRIGGER - Fakes high latency/CPU/RAM to test alarm system
...
- Orchestrator aggregates results + PUBLISHES INFLATED METRICS to test alarms
...
- TELEMETRY VALUES INTENTIONALLY INFLATED TO TRIGGER ALARMS
"""
...
# Separate clients for each VM (for publishing with their own tokens)
vm_mqtt_clients = {
    "VM1": None,
    "VM2": None,
    "VM3": None
...
# ==================== ALARM TRIGGER FLAGS ====================
# Set these to INFLATE telemetry values and trigger alarms

TRIGGER_HIGH_LATENCY = True      # Fakes inference_time_ms > 300ms
TRIGGER_HIGH_CPU = True           # Fakes CPU > 85%
TRIGGER_HIGH_RAM = True           # Fakes RAM > 450MB for VM1, >450MB for others
...
def on_connect(client, userdata, flags, rc):
    ...
    if rc == 0:
        print(f"✅ MQTT Connected (Orchestrator - ALARM TRIGGER MODE)")
        mqtt_connected = True
    else:
...
def initialize_mqtt():
    ...
    try:
        ...
        # Initialize VM clients
        for vm_name in ["VM1", "VM2", "VM3"]:
            vm_token = VMS[vm_name]["token"]
            try:
                client = mqtt.Client(client_id=f"Orchestrator_For_{vm_name}")
                client.username_pw_set(vm_token)
                client.connect(THINGSBOARD_HOST, THINGSBOARD_PORT, keepalive=60)
                client.loop_start()
                vm_mqtt_clients[vm_name] = client
                print(f"   ✅ VM {vm_name} MQTT client initialized")
            except Exception as e:
                print(f"   ⚠️  VM {vm_name} MQTT client failed: {e}")
...
def publish_individual_vm_telemetry(vm_name: str, inference_data: Dict[str, Any]):
    """Publish inflated metrics for individual VMs to trigger their alarms"""
    if vm_name not in vm_mqtt_clients or not vm_mqtt_clients[vm_name]:
        return
    
    try:
        vm_data = inference_data.get("individual_predictions", {}).get(vm_name, {})
        if vm_data.get("status") != "success":
            return
        
        # 🚨 INFLATE METRICS FOR ALARM TESTING 🚨
        inflated_latency = 450.5 if TRIGGER_HIGH_LATENCY else 145.23
        
        # Different CPU/RAM for each VM
        if vm_name == "VM1":
            inflated_cpu = 92.5 if TRIGGER_HIGH_CPU else 0.5
            inflated_ram = 560.8 if TRIGGER_HIGH_RAM else 304.62
        elif vm_name == "VM2":
            inflated_cpu = 88.3 if TRIGGER_HIGH_CPU else 3.95
            inflated_ram = 475.5 if TRIGGER_HIGH_RAM else 417.80
        else:  # VM3
            inflated_cpu = 91.2 if TRIGGER_HIGH_CPU else 4.5
            inflated_ram = 512.3 if TRIGGER_HIGH_RAM else 430.15
        
        telemetry = {
            "vm_id": vm_name,
            "timestamp": datetime.utcnow().isoformat() + "Z",
            "prediction": vm_data.get("prediction", "N/A"),
            "confidence": round(vm_data.get("confidence", 0), 4),
            "inference_time_ms": inflated_latency,
            "cpu_usage_pct": inflated_cpu,
            "ram_usage_mb": inflated_ram,
            "image_name": inference_data.get("image_name", "unknown")
        }
        
        payload = json.dumps(telemetry)
        vm_mqtt_clients[vm_name].publish(THINGSBOARD_TOPIC, payload, qos=1)
    
    except Exception as e:
        pass  # Silent fail

def publish_collective_telemetry(collective_result: Dict[str, Any]):
    """Publish collective diagnosis to ThingsBoard with INFLATED metrics for alarm testing"""
    ...
    try:
        ...
        # 🚨 INFLATE METRICS FOR ALARM TESTING 🚨
        inflated_latency = 450.5 if TRIGGER_HIGH_LATENCY else 145.23      # > 300ms triggers alarm
        inflated_cpu_vm1 = 92.5 if TRIGGER_HIGH_CPU else 0.5              # > 85% triggers alarm
        inflated_cpu_vm2 = 88.3 if TRIGGER_HIGH_CPU else 3.95             # > 85% triggers alarm
        inflated_cpu_vm3 = 91.2 if TRIGGER_HIGH_CPU else 4.5              # > 85% triggers alarm
        inflated_ram_vm1 = 560.8 if TRIGGER_HIGH_RAM else 304.62          # > 450MB triggers alarm
        inflated_ram_vm2 = 475.5 if TRIGGER_HIGH_RAM else 417.80          # > 450MB triggers alarm
        inflated_ram_vm3 = 512.3 if TRIGGER_HIGH_RAM else 430.15          # > 450MB triggers alarm
        ...
        telemetry = {
            ...
            # 🚨 INFLATED INDIVIDUAL METRICS 🚨
            "vm1_latency_ms": inflated_latency,
            "vm2_latency_ms": inflated_latency,
            "vm3_latency_ms": inflated_latency,
            "vm1_cpu_pct": inflated_cpu_vm1,
            "vm2_cpu_pct": inflated_cpu_vm2,
            "vm3_cpu_pct": inflated_cpu_vm3,
            "vm1_ram_mb": inflated_ram_vm1,
            "vm2_ram_mb": inflated_ram_vm2,
            "vm3_ram_mb": inflated_ram_vm3
        }
        ...
        if result.rc == mqtt.MQTT_ERR_SUCCESS:
            alarm_status = "🚨 ALARM TRIGGERS SENT 🚨" if (TRIGGER_HIGH_LATENCY or TRIGGER_HIGH_CPU or TRIGGER_HIGH_RAM) else ""
            print(f"   📤 MQTT: Published with inflated metrics {alarm_status}")
            
            # 🚨 Also publish inflated metrics for EACH individual VM 🚨
            for vm_name in ["VM1", "VM2", "VM3"]:
                publish_individual_vm_telemetry(vm_name, collective_result)
...
def orchestrate_inference(image_path: str, image_name: str, 
    ...
    try:
        ...
        # Publish to ThingsBoard (with inflated metrics)
...
def evaluate_collective_inference(num_test_images: int = 10) -> Dict[str, Any]:
    """Evaluate collective inference system with alarm triggers"""
    print("\n" + "="*80)
    print("🚀 PHASE 5 + 6: COLLECTIVE INTELLIGENCE WITH MQTT TELEMETRY")
    print("🚨 ALARM TRIGGER MODE - Faking High Latency/CPU/RAM 🚨")
    print("="*80)
    
    print("\n🚨 ALARM CONFIGURATION:")
    if TRIGGER_HIGH_LATENCY:
        print("   ⚠️  HIGH LATENCY: inference_time_ms faked to 450ms (threshold: 300ms)")
    if TRIGGER_HIGH_CPU:
        print("   ⚠️  HIGH CPU: All VMs faked to 88-92% (threshold: 85%)")
    if TRIGGER_HIGH_RAM:
        print("   ⚠️  HIGH RAM: All VMs faked to 460-512MB (threshold: 450MB)")
    ...
    print(f"📤 Publishing inflated telemetry to trigger alarms...\n")
    ...
    print(f"\n🚨 ALARMS TRIGGERED:")
    if TRIGGER_HIGH_LATENCY:
        print(f"   ⚠️  {len(successful_results)} latency alarms (450ms / image)")
    if TRIGGER_HIGH_CPU:
        print(f"   ⚠️  {len(successful_results)} CPU alarms (88-92% / image)")
    if TRIGGER_HIGH_RAM:
        print(f"   ⚠️  {len(successful_results)} RAM alarms (460-512MB / image)")
    
    evaluation_report = {
        "timestamp": pd.Timestamp.now().isoformat(),
        "phase": "5+6_mqtt_alarm_test",
        ...
        "alarm_triggers": {
            "high_latency": TRIGGER_HIGH_LATENCY,
            "high_cpu": TRIGGER_HIGH_CPU,
            "high_ram": TRIGGER_HIGH_RAM,
            "total_alarms_sent": len(successful_results) * sum([TRIGGER_HIGH_LATENCY, TRIGGER_HIGH_CPU, TRIGGER_HIGH_RAM])
        },
...
def main():
    """Main execution"""
    print("📡 Initializing MQTT for collective telemetry (ALARM MODE)...")
    initialize_mqtt()
    
    evaluation_report = evaluate_collective_inference(num_test_images=10)
    
    results_path = PROGRESS_DIR / "PHASE5_6_ALARM_TEST_RESULTS.json"
    ...
    for vm_name, client in vm_mqtt_clients.items():
        if client:
            client.loop_stop()
            client.disconnect()
